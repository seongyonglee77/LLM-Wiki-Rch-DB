...
- A summary can become `summarized` only when its evidence JSON declares `summary_depth.level: deep`, supplies a section overview and at least three substantive claims for Theory & Literature Review, Findings/Results, and Discussion, and includes `interpretation` plus `why_it_matters` for every claim. Each claim requires an exact direct quotation. Use a positive page number plus `source_page` when the parsed Markdown provides a reliable page marker; otherwise use a blank page plus `source_text` and require manual review. Never write an unverified paraphrase as a verified citation.
...
- Card YAML keeps `record_id`, `stem`, bibliographic fields, one `tags` list, `summary`, `provenance`, `verification`, and the generated `related` relationship block (`wiki`, `overviews`, `concepts`, `projects`, `questions`, `supersedes`, `superseded_by`). The many-to-many registry at `registry/synthesis-links.json` is canonical. Each paper needs at least one relevant overview or concept; projects and questions are optional. The same relationships are rendered as readable bidirectional links in the card, paper wiki node, and synthesis page.
- Deep-summary evidence is rendered inline under each section and each claim. Do not create a separate `Directly Citable Evidence` table: it duplicates the same quotations and is a deprecated legacy shape. The deep summary must expand, not compress, the literature review/background, findings/results, and discussion/implications.
